...
- `padding`并非必须是四个值，也可以有其他值
  ...
  <img src="assets/image-20220403160517685.png" alt="image-20220403160517685" style="zoom:80%;" />
...
- `margin`单独编写是一个缩写属性：
  ...
  - `margin`的其他值
    ...
    <img src="assets/image-20220403215202698.png" alt="image-20220403215202698" style="zoom:80%;" />
...
- **设置盒子模型中宽高的行为**
  ...
  > 注意事项：`box-sizing`**生效的前提是，有明确的设置宽度和高度**
...
- **设置背景图片的大小**
  - **`auto`（默认）：以背景图自身大小显示**
  - `contain`：缩放背景图，以较长的一边保持宽高比的情况下缩放至完全覆盖铺满元素
  - `cover`：缩放背景图，以较短的一边保持宽高比的情况下缩放至完全覆盖铺满元素，较长的那一边的背景图片部分看不见（超出）
...
- 利用`background-image`和`img`都能够实现显示图片的需求，在开发中该如何选择?
  ...
  <img src="assets/image-20220405224615691.png" alt="image-20220405224615691" style="zoom:80%;" />
