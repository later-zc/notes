...
- `await`关键字有什么特点呢？
  ...
  - 通常使用`await`时后面会跟上一个表达式，这个表达式会返回一个`Promise`对象
...
- 注意：
  - 创建一个`promise`对象时，**传入的`executor`函数是同步的**，会立即执行
  - **`promise`的`then`、`catch`等类方法传入的回调函数，只有在被`resolve`或`reject`调用的时候，才会被加入到微任务队列中**
  - `await`后面的表达式返回的如果是一个`promise`的话，**`await`下面的代码包括await前面的赋值，都会属于该`promise`执行`resolve`产生的微任务中的**，那`reject`呢？因为如果异步函数中`await`后面的表达式返回的是一个`reject`状态的`promise`或报错，此时异步函数后续的代码将不会执行
