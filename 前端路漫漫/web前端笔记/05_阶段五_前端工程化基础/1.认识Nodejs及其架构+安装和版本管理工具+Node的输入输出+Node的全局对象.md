...
- `Window`的同学怎么办？
  - 针对`nvm`，在`GitHub`上有提供对应的`window`版本：`https://github.com/coreybutler/nvm-window`
  - 通过` nvm install latest `安装最新的`node`版本
  - 通过` nvm list `展示目前安装的所有版
  - 通过` nvm use + 指定的nodejs版本号 `切换版本(`window`必须以管理员身份运行的`cmd`)
...
![image-20220721235741651](C:\Users\23634\AppData\Roaming\Typora\typora-user-images\image-20220721235741651.png)
