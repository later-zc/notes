...
<img src="assets/image-20221022153918085.png" alt="image-20221022153918085"  />
...
4. `VSCode`中`eslint`的配置（`vue`项目中不用加，`react`项目中加）
...
- `RADEME.md`：用来写一些文档信息
- `tsconfig.config.json`：当准备修改`tsconfig.json`中的配置时，推荐在此文件中修改，以及`vite`环境相关的ts文件如何进行编译，做了相关的选项
