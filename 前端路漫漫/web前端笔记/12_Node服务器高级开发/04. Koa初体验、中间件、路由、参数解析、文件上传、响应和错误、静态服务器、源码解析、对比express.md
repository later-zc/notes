...
- 实现方案：
  ...
  - `Express` 异步数据的实现
    ...
    app.use(async (req, res, next) => {
      console.log('express middleware 03')
      const result = await '---' // 这里比较特殊，如果是await表达式是一个普通值，不影响后续代码
      req.msg += result
      console.log('111')
...
> **总结：**
>
> - **如果 `next` 执行的下一个中间件是一个异步函数，那么 `next` 默认不会等待下一个中间件的结果，就会执行当前中间件函数中的后续代码**
> - **如果希望等待下一个异步中间件函数的执行结果，`koa` 则需要在 `next` 函数前面加 `await`，而 `express` 即使加上 `await` 也不行**
>
> **注意：**
>
