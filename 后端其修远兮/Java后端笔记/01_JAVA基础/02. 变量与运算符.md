...
<img src="assets/image-20220513235020527.png" alt="image-20220513235020527" style="zoom:50%;" />

- 一花一世界，如果把一个程序看做一个世界或一个社会的话，那么变量就是程序世界的花花草草、万事万物
- 即，**变量是程序中不可或缺的组成单位，最基本的存储单元**

<img src="assets/image-20220513235828042.png" alt="image-20220513235828042" style="zoom: 50%;" />
...
<img src="assets/image-20220520111756274.png" alt="image-20220520111756274" style="zoom: 50%;" />
...
- **`Java` 中的所有字符**都使用 **`Unicode` 编码**，故一个字符可以存储一个字母，一个汉字，或其他书面语的一个字符
...
- 在 `Java` 程序中，不同的基本数据类型变量的值经常需要进行相互转换（只有7种，**不包含 `boolean` 类型**）
...
- 规则：将（取值范围/容量）**小**的类型**自动提升**为（取值范围/容量）**大**的类型 
...
- 规则一：当把存储范围小的值（常量值、变量的值、表达式计算的结果值）赋值给了存储范围大的变量时
...
- 规则二：当存储范围小的数据类型与存储范围大的数据类型变量一起混合运算时，会按照其中最大的类型运算
...
- 规则三：当 `byte、short、char` 数据类型的变量进行算术运算时，按照 `int` 类型处理
...
- 规则：将（取值范围/容量）大的类型强制转换成（取值范围/容量）小的类型
- 自动类型提升是 `Java` 自动执行的，而强制类型转换是自动类型提升的逆运算，需要手动执行

- 转换格式：

  > // T：数据类型，**T的取值范围 <= 变量值的数据类型的取值范围**
  >
  > **T 变量名 = (T)变量值** 

- 规则一：当把存储范围大的值（常量值、变量的值、表达式计算的结果值）强制转换为存储范围小的变量时，可能会造成**精度损失**或**溢出**

  ```java
  int i = 3.14; // 错误: 不兼容的类型: 从double转换到int可能会有损失
  int i = (int)3.14; // 不报错，但可能会有损失精度
  
  double d = 1.2;
  int num = d; // 错误: 不兼容的类型: 从double转换到int可能会有损失
  int num = (int)d; // 不报错，但可能会有损失精度
...
- 规则二：当某个值**想要提升数据类型时**，也可以使用强制类型转换。这种情况的强制类型转换是**没有风险**的，通常**省略**
...
- 声明 `long` 类型变量时，可以出现省略后缀的情况。`float` 则必须加上后缀
